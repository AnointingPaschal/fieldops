...
<head>
  ...
  <title>FieldOps Report — Alberta Safety Control</title>
</head>
  ...
  <div style="max-width:640px;margin:32px auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.08);">
    ...
    <div style="background:#0B1D35;padding:32px 40px;position:relative;overflow:hidden;">
      ...
      <div style="margin-top:28px;">
        <div style="color:rgba(255,255,255,0.5);font-size:12px;margin-bottom:6px;">Good day — here's your operations summary</div>
        <div style="color:white;font-size:22px;font-weight:900;line-height:1.2;">Field Operations<br/>Summary Report</div>
      </div>
    </div>

    <!-- Stats band -->
    <div style="background:#1D4ED8;padding:0;">
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          ${[
          ...
          <td style="padding:18px 10px;text-align:center;border-right:1px solid rgba(255,255,255,0.1);">
            <div style="color:rgba(255,255,255,0.6);font-size:18px;margin-bottom:4px;">${s.icon}</div>
            <div style="color:white;font-size:24px;font-weight:900;line-height:1;">${s.value}</div>
            <div style="color:rgba(255,255,255,0.55);font-size:10px;margin-top:4px;text-transform:uppercase;letter-spacing:0.05em;">${s.label}</div>
          </td>`).join('')}
        </tr>
      </table>
    </div>
    ...
    <div style="padding:32px 40px;">
      ...
      <div style="margin-bottom:32px;">
        ...
        <div style="border-radius:12px;overflow:hidden;border:1px solid #E2E8F0;">
          <table style="width:100%;border-collapse:collapse;">
            <thead>
              <tr style="background:#F8FAFC;">
                <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #E2E8F0;">Contractor</th>
                <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #E2E8F0;">Type</th>
                <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #E2E8F0;">Status</th>
                <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #E2E8F0;">Completed</th>
              </tr>
            </thead>
          ...
          ${tasks.length > 10 ? `<div style="padding:12px 16px;background:#F8FAFC;text-align:center;font-size:12px;color:#64748B;border-top:1px solid #E2E8F0;">+${tasks.length - 10} more tasks — log in to view all</div>` : ''}
        </div>
      </div>
      ...
      <div style="margin-bottom:32px;">
        <div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:12px;padding:16px 20px;margin-bottom:16px;display:flex;align-items:center;gap:12px;">
          ...
          <div>
            ...
            <div style="font-size:12px;color:#B91C1C;">${discrepancies.length} item(s) with missing or damaged status — follow up with contractors</div>
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;">
            <thead>
              <tr style="background:#FEF2F2;">
                <th style="padding:9px 14px;text-align:left;font-size:11px;font-weight:700;color:#DC2626;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #FECACA;">Contractor</th>
                <th style="padding:9px 14px;text-align:left;font-size:11px;font-weight:700;color:#DC2626;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #FECACA;">Item</th>
                <th style="padding:9px 14px;text-align:center;font-size:11px;font-weight:700;color:#D97706;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #FECACA;">Damaged</th>
                <th style="padding:9px 14px;text-align:center;font-size:11px;font-weight:700;color:#DC2626;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid #FECACA;">Not Found</th>
              </tr>
            </thead>
      ...
      <div style="text-align:center;margin-bottom:32px;">
        <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://fieldops.vercel.app'}/supervisor/dashboard"
          style="display:inline-block;background:#0B1D35;color:white;font-size:14px;font-weight:800;padding:14px 36px;border-radius:12px;text-decoration:none;letter-spacing:0.02em;">
          View Full Dashboard →
        </a>
      </div>
    </div>
    ...
    <div style="background:#F8FAFC;border-top:1px solid #E2E8F0;padding:24px 40px;text-align:center;">
      ...
      <div style="font-size:10px;color:#CBD5E1;line-height:1.6;">
        ...
        You're receiving this because you're listed as a report recipient.<br/>
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Fetch Mailjet settings
    const { data: settingsRows } = await db
      .from('app_settings')
      .select('key, value')
      .in('key', ['mailjet_api_key', 'mailjet_secret_key', 'mailjet_from_email', 'mailjet_from_name']);

    const settings: Record<string, string> = {};
    (settingsRows || []).forEach((r: any) => { settings[r.key] = r.value; });

    if (!settings.mailjet_api_key || !settings.mailjet_secret_key) {
    ...
    const recipients: string[] = isTest
      ? [body.testEmail || settings.mailjet_from_email]
      : (schedule?.recipients || []);
    ...
    const [{ data: tasks }, { data: workers }, { data: discrepancies }] = await Promise.all([
      ...
      db.from('task_item_recovery')
        .select('*,item:inventory_items(name),task:tasks(contractor:contractors(name))')
        .or('quantity_damaged.gt.0,quantity_missing.gt.0'),
    ]);

    const now    = new Date();
    const period = now.toLocaleDateString('en-CA',{ month:'long', day:'numeric', year:'numeric' });
    const html   = buildEmailHtml({ period, tasks: tasks||[], workers: workers||[], discrepancies: discrepancies||[] });
    const subject = isTest
      ? `[TEST] FieldOps Report — ${period}`
      : `FieldOps Operations Report — ${period}`;
    ...
    const mjRes = await fetch('https://api.mailjet.com/v3.1/send', {
      ...
      body: JSON.stringify({
        Messages: recipients.map(email => ({
          From: {
            Email: settings.mailjet_from_email || 'reports@albertasafetycontrol.com',
            Name:  settings.mailjet_from_name  || 'Alberta Safety Control',
          },
          To: [{ Email: email }],
          Subject: subject,
          HTMLPart: html,
    ...
    if (!mjRes.ok) {
      const err = await mjRes.json();
      return NextResponse.json({ error: 'Mailjet error', detail: err }, { status: 500 });
    }
