...
import {
  ...
  ChevronRight, Navigation, Image as ImageIcon,
} from 'lucide-react';
...
export default function TaskDetailPage() {
  ...
  const [modal,    setModal]    = useState<'note' | 'photo' | 'issue' | 'complete' | null>(null);
  ...
  const handleStatusChange = async (nextStatus: string) => {
    if (!task || !user) return;
    if (nextStatus === 'Completed') { setModal('complete'); return; }
    setActing(true);
  ...
  const handleAddUpdate = async (type: 'note' | 'photo' | 'issue') => {
    ...
    setModal(null); setText(''); setFile(null); setPreview('');
    showToast(type === 'note' ? 'Note added.' : type === 'photo' ? 'Photo uploaded.' : 'Issue reported.');
    load();
    setActing(false);
  };
  ...
  return (
    <AppShell role="worker" userName={user?.name || 'Worker'}>
      <div className="space-y-4 max-w-2xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-text-muted">View and update task status</p>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-text-muted">Created {new Date(task.created_at).toLocaleDateString('en-CA',{ month:'short', day:'numeric' })}</p>
              </div>
            </div>
          ...
          {nextAction && canAct && (
            <motion.button whileTap={{ scale:0.97 }} onClick={() => handleStatusChange(nextAction.next)}
              ...
              {acting ? <Loader2 className="w-4 h-4 animate-spin" /> : <><CheckCircle className="w-4 h-4" />{nextAction.label}</>}
            </motion.button>
          )}
        ...
        <div className="card space-y-3">
          <p className="sec-title">Contractor &amp; Location</p>
            ...
            <div className="flex-1 min-w-0">
              ...
              {task.contractor?.address && (
                <div className="flex items-start gap-1.5 mt-1">
                  ...
                  <p className="text-[12px] text-text-secondary">{task.contractor.address}</p>
                </div>
              )}
            </div>
          </div>
          {task.contractor?.phone && (
            <a href={`tel:${task.contractor.phone}`}
              className="flex items-center gap-2 text-[12px] text-sky font-semibold hover:underline">
          ...
          {task.contractor?.contact_name && (
            <p className="text-[12px] text-text-muted flex items-center gap-1.5">
              <Users className="w-3.5 h-3.5" /> Contact: {task.contractor.contact_name}
            </p>
          )}
          {/* Live map */}
          {task.contractor?.address && (
            <Map address={task.contractor.address} label={task.contractor.name || 'Site'} height={200} />
          )}
        </div>
              ...
              {task.rental_start && (
                <div className="bg-slate-50 rounded-lg p-3">
                  ...
                  <p className="font-semibold text-[12px] text-text-primary">
                    {new Date(task.rental_start).toLocaleDateString('en-CA',{ month:'short', day:'numeric', year:'numeric' })}
                  </p>
                  <p className="text-[11px] text-text-muted">
                    {new Date(task.rental_start).toLocaleTimeString('en-CA',{ hour:'2-digit', minute:'2-digit' })}
                  </p>
                </div>
              )}
              {task.rental_end && (
                <div className="bg-warn/5 border border-warn/20 rounded-lg p-3">
                  ...
                  <p className="font-semibold text-[12px] text-text-primary">
                    {new Date(task.rental_end).toLocaleDateString('en-CA',{ month:'short', day:'numeric', year:'numeric' })}
                  </p>
                  <p className="text-[11px] text-warn">
                    {new Date(task.rental_end).toLocaleTimeString('en-CA',{ hour:'2-digit', minute:'2-digit' })}
                  </p>
                </div>
              )}
            </div>
        ...
        {task.items && task.items.length > 0 && (
          <div className="card">
            <p className="sec-title mb-3">Equipment ({task.items.length} item{task.items.length !== 1 ? 's' : ''})</p>
        ...
        {canAct && (
          <div>
            <p className="sec-title mb-2">Actions</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { icon: Camera,      label:'Photo',  action: () => setModal('photo'), c:'text-warn', bg:'bg-warn/10' },
                { icon: FileText,    label:'Note',   action: () => setModal('note'),  c:'text-pass', bg:'bg-pass/10' },
                { icon: AlertCircle, label:'Issue',  action: () => setModal('issue'), c:'text-fail', bg:'bg-fail/10' },
              ].map(({ icon: I, label, action, c, bg }) => (
              ...
              {updates.map((u: any) => (
                <motion.div key={u.id} initial={{ opacity:0 }} animate={{ opacity:1 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {u.photo_url && <img src={u.photo_url} alt="update" className="w-full rounded-lg mt-2 max-h-48 object-cover" />}
      ...
      {/* ── Modals ── */}
      <AnimatePresence>
        {(modal === 'note' || modal === 'photo' || modal === 'issue' || modal === 'complete') && (
            ...
            <motion.div initial={{ y:24, opacity:0 }} animate={{ y:0, opacity:1 }}
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-line">
                <h3 className="font-bold text-text-primary text-[15px]">
                  {modal === 'note'     ? 'Add Note'
                  : modal === 'photo'   ? 'Upload Photo'
                  : modal === 'issue'   ? 'Report Issue'
                  :                       'Complete Task'}
                </h3>
              ...
              <div className="p-5 space-y-3">
                ...
                {(modal === 'photo' || modal === 'complete') && (
                  <div onClick={() => fileRef.current?.click()}
                    className="w-full h-36 rounded-xl border-2 border-dashed border-line hover:border-sky/50 bg-slate-50 cursor-pointer flex items-center justify-center overflow-hidden relative transition-colors">
                    {preview
                      ? <img src={preview} alt="preview" className="w-full h-full object-cover" />
                      : <div className="flex flex-col items-center gap-1.5 text-text-muted">
                          <Camera className="w-6 h-6" />
                          <p className="text-[12px] font-medium">
                            {modal === 'complete' ? 'Take completion photo (required)' : 'Tap to take / choose photo'}
                          </p>
                        </div>}
                ...
                <textarea className="textarea" rows={3}
                  placeholder={
                    modal === 'note'     ? 'Write your note…'
                    : modal === 'photo'  ? 'Caption (optional)…'
                    : modal === 'issue'  ? 'Describe the issue…'
                    :                      'Any final notes before completing?'
                  }
